...
Функционал: Загрузка документа РасходТовара для проверки печатной формы

Сценарий: Загрузка документа РасходТовара для проверки печатной формы
	// Справочник.Валюты
	...
	И я проверяю или создаю для справочника "Товары" объекты:
		...
		| 'e1cib/data/Справочник.Товары?ref=8ca2000d8843cd1b11dc9111f169782e' | 'False'           | 'e1cib/data/Справочник.Товары?ref=a100005056c0000811e3f7c0ef680671' | 'False'     | '000000035' | 'Йогурт'       | 'S564'    | ''                                                                       | ''             | 'Enum.ВидыТоваров.Товар' | ''         |
	...
	И я проверяю или создаю для документа "РасходТовара" объекты:
		...
		| 'e1cib/data/Документ.РасходТовара?ref=935b60dd8ed5021d11ee57d08bc105c4' | 'False'           | '912      ' | '20.09.2023 20:09:18' | 'False'    | 'e1cib/data/Справочник.Контрагенты?ref=8ca0000d8843cd1b11dc8d043d710076' | 'e1cib/data/Справочник.Склады?ref=a9b000055d49b45e11db8b8bdc1aadc2' | 'e1cib/data/Справочник.Валюты?ref=a9b000055d49b45e11db8c4421dda1c5' | 'e1cib/data/Справочник.ВидыЦен?ref=a9b000055d49b45e11db8c4c9d5c4225' | ''                    | 'e1cib/data/Справочник.Организации?ref=8d34000d8843cd1b11dd2bea12f94c63' |

	И я перезаполняю для объекта табличную часть "Товары":
		| 'Ссылка'                                                                | 'Товар'                                                             | 'Цена' | 'Количество' | 'Сумма' |
		| 'e1cib/data/Документ.РасходТовара?ref=935b60dd8ed5021d11ee57d08bc105c4' | 'e1cib/data/Справочник.Товары?ref=8ca2000d8843cd1b11dc9111f169782e' | 35     | 5            | 175     |
		| 'e1cib/data/Документ.РасходТовара?ref=935b60dd8ed5021d11ee57d08bc105c4' | 'e1cib/data/Справочник.Товары?ref=8ca1000d8843cd1b11dc8ea92d97a749' | 300    | 10           | 3000    |
